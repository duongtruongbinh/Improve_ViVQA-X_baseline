...
def _get_question_type_specialized(question_text: str) -> str:
...
async def run_vqa_pipeline(
    ...
    determined_question_type = _get_question_type_specialized(question)
    ...
    response_data = {
        ...
        "question_type": determined_question_type,
        "initial_answer": "",
        "final_answer": "",
    ...
    if any(agent is None for agent in agent_instances_list) or any(client is None for client in client_instances_list):
        ...
        direct_check_res, grades_res, mj_vote_res = perform_direct_accuracy_check(
            response_data["final_answer"], target_answer, response_data["question_type"], response_data["error"], verbose
        )
        response_data["direct_accuracy_check"] = direct_check_res
        response_data["grades"] = grades_res
        response_data["majority_vote"] = mj_vote_res
        return response_data

    if question is None:
        question = ""
        response_data["question"] = ""
        response_data["question_type"] = _get_question_type_specialized(question)
    ...
    try:
        ...
        if valid_image_url_for_message:
            ...
            response_data["majority_vote"] = mj_vote_res
            return response_data
        initial_user_message = {"role": "user", "content": initial_vlm_message_parts}
        ...
        chat_result_initial = await vqa_orchestrator.a_initiate_chat(
            recipient=initial_vlm_agent, message=initial_user_message,
    ...
    is_direct_failure_marker = (
        "[Answer Failed]" in initial_answer_text or "sorry" in initial_answer_text.lower() or
        "unable to answer" in initial_answer_text.lower() or "cannot answer" in initial_answer_text.lower() or
        not initial_answer_text.strip() or "[Agent Communication Error]" in initial_answer_text or
        "[Agent Call Failed]" in initial_answer_text or "[Initial VLM Agent returned empty message]" in initial_answer_text
    )
    ...
    if inference_settings.get("force_multi_agents", False) and not response_data.get("error"):
        if not match_baseline_failed and verbose: print("   Note: force_multi_agents=True, forcing reattempt.")
    ...
    if match_baseline_failed and response_data["error"] is None:
        ...
        try:
            ...
            failure_analysis_user_message_str = (
                f"Original Question: '{question}'\nInitial VLM Response: '{initial_answer_text}'\n"
        ...
        if is_numeric_reattempt:
            ...
            numeric_reattempt_user_prompt = (
                f"Original question: '{question}'\nPrevious Answer: '{initial_answer_text}'\n"
                f"Analysis suggests issues with counting: '{item_to_count}'.\n"
                "INSTRUCTION: Look carefully. Count step-by-step how many "
                f"'{item_to_count}' are present. List instances if possible. "
                "Respond starting with '[Reattempted Answer]' followed ONLY by the final number/answer."
            )
            numeric_reattempt_message_parts = [{"type": "text", "text": numeric_reattempt_user_prompt}]
            if valid_image_url_for_message: numeric_reattempt_message_parts.append({"type": "image_url", "image_url": {"url": valid_image_url_for_message}})
            else:
                if verbose: print("   WARNING: Valid image URL not available for reattempt.")
            numeric_reattempt_user_message = {"role": "user", "content": numeric_reattempt_message_parts}
            ...
            try:
                ...
                object_attribute_user_prompt = (
                    f"Question: '{question}'\nFocus on: '{textually_specified_objects}'\n"
                    "Describe these objects and their relevant attributes in detail. If not visible, state so clearly."
                )
                object_attribute_message_parts = [{"type": "text", "text": object_attribute_user_prompt}]
                if valid_image_url_for_message: object_attribute_message_parts.append({"type": "image_url", "image_url": {"url": valid_image_url_for_message}})
                else:
                    if verbose: print("   WARNING: Valid image URL not available for object attribute query.")
                object_attribute_user_message = {"role": "user", "content": object_attribute_message_parts}
            ...
            if "[Attribute Query Agent Error]" not in object_attributes_descriptions and "[Attribute Query Agent Call Failed]" not in object_attributes_descriptions:
                ...
                final_reattempt_user_prompt = (
                    ...
                    "INSTRUCTION: Using this new information, answer the question. "
                    "Start with '[Reattempted Answer]' followed by your answer."
                )
                final_reattempt_message_parts = [{"type": "text", "text": final_reattempt_user_prompt}]
                if valid_image_url_for_message: final_reattempt_message_parts.append({"type": "image_url", "image_url": {"url": valid_image_url_for_message}})
                else:
                    if verbose: print("   WARNING: Valid image URL not available for final reattempt.")
                final_reattempt_user_message = {"role": "user", "content": final_reattempt_message_parts}
                try:
                    reattempt_vlm_agent.reset()
