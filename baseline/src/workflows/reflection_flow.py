...
async def run_simple_vqa_pipeline(
    ...
    agent.reset()
    agent.prepare_system_message(question_text=question)
    ...
    try:
        ...
        if not generated_content_str:
            ...
            try:
                gen_output = json.loads(generated_content_str)
                generated_answer = str(gen_output.get("answer", "")).strip()
                if not generated_answer:
                    generated_answer = "[VQAOutputEmptyAnswerInJSON]"
                    logger.warning(f"VQA Agent (QID: {question_id}) output valid JSON but 'answer' field was empty.")
            except json.JSONDecodeError:
                logger.warning(f"VQA Agent (QID: {question_id}) output was NOT valid JSON: '{generated_content_str[:200]}...'. Using raw output as answer.")
                generated_answer = generated_content_str.strip()
        
        logger.info(f"VQA Agent Output (QID: {question_id}): Answer='{generated_answer[:100]}...'")
    ...
    logger.info(f"--- Simple VQA Pipeline END for QID: {question_id}, Final Answer: '{str(response_data['final_answer'])[:100]}...', Time: {response_data['processing_time_seconds']:.2f}s ---")
    return response_data