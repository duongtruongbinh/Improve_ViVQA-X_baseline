...
MAX_QUESTIONS_TO_PROCESS = 10000 
...
def perform_direct_accuracy_check(
    ...
    if not cleaned_model_ans and cleaned_model_ans != "0":
...
def main():
    print(f"Starting VQA processing for split: {DATASET_SPLIT_NAME}")
    ...
    result_file_name = f"VQAv2_{DATASET_SPLIT_NAME}_{VLLM_MODEL_NAME.split('/')[-1]}_vllm_base64_eval.json"
    result_file_path = os.path.join(OUTPUT_DIR, result_file_name)

    val_data_full = []
    if DATASET_SPLIT_NAME == "rest-val":
        print(f"Loading rest-val questions from: {REST_VAL_QUESTIONS_PATH}")
        print(f"Loading rest-val annotations from: {REST_VAL_ANNOTATIONS_PATH}")
        try:
            with open(REST_VAL_QUESTIONS_PATH, 'r', encoding='utf-8') as f_q:
                questions_data_json = json.load(f_q)
                questions_data = questions_data_json.get('questions', [])
            with open(REST_VAL_ANNOTATIONS_PATH, 'r', encoding='utf-8') as f_a:
                annotations_map = json.load(f_a)
        except FileNotFoundError as e:
            print(f"ERROR: One of the rest-val files not found: {e}")
            return
        except json.JSONDecodeError as e:
            print(f"ERROR: Could not read rest-val JSON file: {e}")
            return

        for q_item in questions_data:
            question_id = q_item.get('question_id')
            ann_item = annotations_map.get(str(question_id)) # VQA annotations often use string qid keys

            if ann_item:
                combined_item = {
                    'question_id': question_id,
                    'question': q_item.get('question'),
                    'image_id': q_item.get('image_id'),
                    'answers': ann_item.get('answers', []),
                    'answer_type': ann_item.get('answer_type', 'other').lower(),
                    'multiple_choice_answer': ann_item.get('multiple_choice_answer')
                }
                val_data_full.append(combined_item)
            # else:
                # print(f"Warning: No annotation found for question_id {question_id} in rest-val.")
        
        if not val_data_full:
            print("ERROR: No data loaded for rest-val after combining questions and annotations.")
            return
    else:
        print(f"ERROR: DATASET_SPLIT_NAME '{DATASET_SPLIT_NAME}' is not configured for loading in this script.")
        print("Please set DATASET_SPLIT_NAME to 'rest-val' or add logic for other splits.")
        return
    ...
    if MAX_QUESTIONS_TO_PROCESS is not None and MAX_QUESTIONS_TO_PROCESS > 0 and MAX_QUESTIONS_TO_PROCESS < len(val_data_full):
        ...
        print(f"Processing the first {MAX_QUESTIONS_TO_PROCESS} questions for {DATASET_SPLIT_NAME}.")
    else:
        val_data = val_data_full
        print(f"Processing all {len(val_data)} questions from the {DATASET_SPLIT_NAME} split.")
    ...
    for i in tqdm(range(0, len(val_data), PROCESSING_CHUNK_SIZE), desc=f"Processing {DATASET_SPLIT_NAME} batches"):
        chunk = val_data[i:i + PROCESSING_CHUNK_SIZE]
        questions = [item['question'] for item in chunk]
        
        image_paths = []
        for item in chunk:
            # VQA-v2 images are typically in val2014 for val/rest-val splits
            image_filename = f"COCO_val2014_{item['image_id']:012d}.jpg"
            image_paths.append(os.path.join(VAL_IMAGE_DIR, image_filename))
        ...
        for item_data, model_predicted_answer in zip(chunk, predicted_batch_answers):
            ...
            gt_answers_list = item_data.get('answers', []) 
            question_type_from_data = item_data.get('answer_type', 'other').lower() # Ensure this key exists
            current_question_id = item_data['question_id']

            is_item_correct = False
            best_match_notes = "No match with any ground truth answers."
            
            # If gt_answers_list is empty, but multiple_choice_answer exists, use it
            eval_targets = gt_answers_list
            if not eval_targets and item_data.get('multiple_choice_answer'):
                eval_targets = [{'answer': item_data['multiple_choice_answer']}]


            if not eval_targets:
                best_match_notes = "No ground truth answers provided for evaluation."
            else:
                for gt_entry in eval_targets:
                    gt_text = gt_entry.get('answer')
                    if gt_text is None: continue # Skip if an answer entry is malformed
                    ...
                    if accuracy_results.get("is_correct"):
                        ...
                        best_match_notes = accuracy_results.get("notes", "Match with one GT.")
                        break 
            ...
            if (total_evaluated_questions % 100 == 0 or total_evaluated_questions < 10) : # Print more frequently at start
                 print(f"QID {current_question_id}: Type='{question_type_from_data}', Pred='{model_predicted_answer}', GTs={len(eval_targets)}, Correct={is_item_correct}, Note='{best_match_notes}'")
            ...
            final_results_with_eval.append({
                ...
                'gt_ans_struct': eval_targets, # Store the GTs used for eval
    ...
    if total_evaluated_questions > 0:
        ...
        print(f"\n--- Overall Evaluation Statistics ({DATASET_SPLIT_NAME}) ---")
        print(f"Total questions evaluated: {total_evaluated_questions}")
        print(f"Correct predictions: {correct_predictions_count}")
        print(f"Overall accuracy: {overall_accuracy:.2f}%")

        print(f"\n--- Accuracy Statistics By Question Type ({DATASET_SPLIT_NAME}) ---")
        sorted_q_types = sorted(total_by_type.keys())
