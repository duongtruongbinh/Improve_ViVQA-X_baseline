...
class VQATask(BaseTask):
    ...
    def valid_step(self, model, samples):
        # answers, candidates_dict_list = model.predict_answers(
        #     samples=samples,
        ...
        #     prompt=self.prompt,
        # )
        # # print('candidates_dict_list', candidates_dict_list)
        # pred_qa_pairs = []

        # question_id = samples["question_id"]
        # gt_answers = samples["answer_list"]
        # question_texts = samples['text_input']
        # image_names = samples['image_name']
        
        # for answer, ques_id, image_name ,question_text ,gt_answer, candidates_dict in zip(answers, question_id,image_names ,question_texts,gt_answers, candidates_dict_list):
        #     ques_id = int(ques_id.item())
        #     pred_qa_pairs.append({"question_id": ques_id, "question_text": question_text ,"image_name":image_name,"answer": answer, "gt_ans": gt_answer, 'candidates_dict':candidates_dict})
    ...
    def _report_metrics(self, result_file, split):
        ...
        if split in self.ques_files and split in self.anno_files:
            print('annotion_file', self.anno_files[split])
            print('question_file', self.ques_files[split])
            print('resFile', result_file)
            print('quesFile', self.ques_files[split])
...
class GQATask(VQATask):
    def valid_step(self, model, samples):
        ...
        for answer, ques_id, image_name ,question_text ,gt_answer, candidates_dict in zip(answers, question_id,image_names ,question_texts,gt_answers, candidates_dict_list):
            ques_id = int(ques_id.item())
            pred_qa_pairs.append({"question_id": ques_id, "question_text": question_text ,"image_name":image_name,"pred_ans": answer, "gt_ans": gt_answer, 'candidates_dict':candidates_dict})
