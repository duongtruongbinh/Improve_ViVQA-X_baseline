...
class BlipVQA(BlipBase):
    ...
    def _rank_answers(self, samples, answer_list, num_ans_candidates):
        ...
        """
        # num_ans_candidates =5  # debug
        print('num_ans_candidates:', num_ans_candidates)
        ...
        print('question_atts:', question_atts)
        num_ques = question_states.size(0)
        start_ids = answer_ids[0, 0].repeat(num_ques, 1)  # bos token

        print('--------------------------before first decoder--------------------------')
        print('start_ids:', start_ids)
        print('encoder_hidden_states:', question_states)
        print('encoder_attention_mask:', question_atts)
        # print('labels:', targets_ids)
        ...
        print('WZQ start_output.logits:', start_output.logits.shape)
        print('WZQ Logits:', logits, logits.shape)
        # topk_probs: top-k probability
        # topk_ids: [num_question, k]
        print('WZQ Answer IDs:', answer_ids, answer_ids.shape)
        answer_first_token = answer_ids[:, 1]
        print('WZQ answer_first_token:', answer_first_token)
        
        
        prob_first_token = F.softmax(logits, dim=1).index_select(
            dim=1, index=answer_first_token
        )
        print('WZQ prob_first_token:', prob_first_token)
        topk_probs, topk_ids = prob_first_token.topk(num_ans_candidates, dim=1)
        print('WZQ  topk_ids:',  topk_ids)
        ...
        print('--------------------------before secound decoder--------------------------')
        print('Inputs ids:', input_ids)
        print('attention mask:', input_atts)
        print('encoder hidden states:', question_states)
        print('encoder_attention_mask:', question_atts)
        print('labels:', targets_ids)
        ...
        print('log_probs_sum shape:', log_probs_sum.size())
        max_topk_ids = log_probs_sum.argmax(dim=1)
        max_ids = topk_ids[max_topk_ids >= 0, max_topk_ids]

        answers = [answer_list[max_id] for max_id in max_ids]
        print('log_probs_sum', log_probs_sum.shape)
        print('num_ans_candidates_:', num_ans_candidates)
        print('max_idxs',max_ids)
        print('output: ', output)
        print('logits size:', output.logits.size())
        return answers
