...
import os
class VQAEval():
    def __init__(self, vqa_eval_res=None, n=2):
        ...
        self.evalQuesType = {}
        self.evalAnsType = {}
        self.vqa_eval_res = vqa_eval_res
        # self.vqa = vqa
        # self.vqaRes = vqaRes
        # if vqa is not None:
    ...
    def evaluate_wzq(self, quesIds=None):
        # may error
            ...
            for quesId in quesIds:
                ...
                # assert False
                gtAcc.append(acc)
                if "question_type" not in gts[quesId]:
                    gts[quesId]["question_type"] = 'question_dump_type'
                if "answer_type" not in gts[quesId]:
                    gts[quesId]["answer_type"] = 'answer_dump_type'
                quesType = gts[quesId]["question_type"]
                ansType = gts[quesId]["answer_type"]
                avgGTAcc = float(sum(gtAcc)) / len(gtAcc)
                accQA.append(avgGTAcc)
