...
def get_nlg_scores(references, hypotheses, device='cuda'):
    ...
    P, R, F1 = bert_score(
        hypotheses, [refs[0] for refs in references], lang='en', device=device)
...
def evaluate(json_file, output_file="evaluation_scores.json", device='cuda'):
    ...
    for item in data:
        gt_ans = item['ground_truth'].strip().lower()
        pred_ans = item['final_answer'].strip().lower()

        # Lấy danh sách explanation ground truth (bạn có thể xử lý thêm nếu cần)
        gt_expls = item['ground_truth_explanations']

        # Loại bỏ tiền tố "because" (hoặc "vì") ở lời giải thích dự đoán nếu có
        pred_expl = item['explanation'].strip().lower()
...
if __name__ == "__main__":
    evaluate("../../output/results.json",
             output_file="../../output/results_scores.json", device='cuda')