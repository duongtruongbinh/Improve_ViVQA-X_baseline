...
class SynthesizerEngine:
    ...
    def _check_hypothesis_conditions(self, hypothesis: Dict, evidence_map: Dict[str, str]) -> bool:
        ...
        # Debug logging for hypothesis validation
        self.logger.debug(f"Checking hypothesis {hypothesis.get('hypothesis_id')} with {len(conditions)} conditions")
        self.logger.debug(f"Available evidence IDs: {list(evidence_map.keys())}")
        ...
        for i, condition in enumerate(conditions):
            ...
            # Debug logging for condition details
            self.logger.debug(f"Condition {i}: {condition}")
            self.logger.debug(f"Extracted evidence_id: {evidence_id}, required_answer: {required_answer_raw}")
            ...
            self.logger.debug(f"Comparing: actual='{actual_answer}' vs required='{required_answer}'")
