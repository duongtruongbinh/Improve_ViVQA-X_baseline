...
def save_sample_result(result, output_dir, sample_type, sample_id):
    ...
    text_info = {
        ...
        "evidence": result.get('causal_trace', []),
        "hypothesis": result.get('hypothesis_count', 0),
...
def create_output_visualization(result, sample_dir):
    """Create a visualization of the output with question, answer, and explanation"""
    ...
    # Question
    question = result.get('question', '')
    ax2.text(0.05, 0.95, f"Question: {question}", 
            fontsize=12, fontweight='bold', transform=ax2.transAxes,
            verticalalignment='top', wrap=True)
    
    # Answer
    answer = result.get('final_answer', '')
    ax2.text(0.05, 0.85, f"Answer: {answer}", 
            fontsize=12, fontweight='bold', color='green', transform=ax2.transAxes,
            verticalalignment='top', wrap=True)
    
    # Ground truth
    ground_truth = result.get('ground_truth', '')
    ax2.text(0.05, 0.75, f"Ground Truth: {ground_truth}", 
            fontsize=12, fontweight='bold', color='blue', transform=ax2.transAxes,
            verticalalignment='top', wrap=True)
    
    # Explanation
    explanation = result.get('explanation', '')
    ax2.text(0.05, 0.55, f"Explanation: {explanation}", 
    ...
    ax2.text(0.05, 0.35, f"Evidence: {evidence_count}, Hypothesis: {hypothesis_count}", 
            fontsize=10, color='gray', transform=ax2.transAxes,
            verticalalignment='top')
    
    # Status
    status = result.get('synthesis_status', '')
    ax2.text(0.05, 0.25, f"Status: {status}", 
            fontsize=10, color='orange', transform=ax2.transAxes,
            verticalalignment='top')
