...
def load_config(config_path: str) -> dict:
    ...
    try:
        with open(config_path, 'r', encoding='utf-8') as f:
            ...
            print(
                f"✓ Successfully loaded config with keys: {list(config.keys()) if config else 'None'}")
            return config
...
def setup_output_directory(args: dict, mode: str) -> str:
    ...
    output_dir = os.path.join(
        module_x_dir, "outputs", f"vivqa_x_{mode}_{dataset_split}_{model_name}_{timestamp}")
    ...
    try:
        ...
        if not answer or not explanation:
            answer_match = re.search(
                r'Answer:\s*(.*)', response_text, re.IGNORECASE | re.DOTALL)
            if answer_match:
                answer = answer_match.group(1).strip().split('\n')[0]

            explain_match = re.search(
                r'Explain:\s*(.*)', response_text, re.IGNORECASE | re.DOTALL)
            if explain_match:
        ...
        if not answer and ("Kết luận:" in response_text or "kết luận:" in response_text):
            if "Kết luận:" in response_text:
                answer = response_text.split("Kết luận:")[
                    1].strip().split('\n')[0]
...
def vlm_only_process_item(data_item: dict, args: dict, session: requests.Session, verbose: bool = False) -> dict:
    """
    Process a single VQA-X item using only the VLM to get a baseline answer.
    """
    ...
    try:
        ...
        # VLM system prompt for direct answering
        vlm_direct_prompt_system = (
            "Bạn là một trợ lý AI xuất sắc trong việc trả lời câu hỏi về hình ảnh (Visual Question Answering).\n"
            "Nhiệm vụ của bạn là trả lời bằng tiếng Việt câu hỏi dựa trên hình ảnh được cung cấp.\n"
            "Bạn phải tuân thủ TUYỆT ĐỐI các quy tắc định dạng đầu ra dưới đây.\n\n"
            "--- QUY TẮC BẮT BUỘC ---\n"
            "1. 'Answer:' Chỉ chứa MỘT TỪ hoặc MỘT CỤM TỪ ngắn gọn duy nhất là câu trả lời cho câu hỏi.\n"
            "2. 'Explain:' Lời giải thích ngắn gọn cho câu trả lời, dựa trên bằng chứng thị giác trong hình."
        )

        messages = [
            {
                "role": "system",
                "content": [{"type": "text", "text": vlm_direct_prompt_system}]
            },
            {
                "role": "user",
                "content": [
        ...
        raw_response = query_vllm_api(messages, args, session)
        answer, explanation = parse_coordinator_response(raw_response)
        ...
        return {
            ...
            'multiagent_triggered': False,  # Mark that this was a vlm-only run
            'agent_results': [],
        ...
        if verbose:
            print(f"\nError in VLM-only processing: {str(e)}")
...
def multi_agent_process_item(data_item: dict, args: dict, session: requests.Session, verbose: bool = False) -> dict:
    ...
    try:
        ...
        llm_result = llm_agent_reasoning(
            question, visual_analysis, args, session)
        ...
        if not coordinator_result['success']:
            raise Exception(
                f"Coordinator Agent failed: {coordinator_result['output']}")

        # Parse final response
        answer, explanation = parse_coordinator_response(
            coordinator_result['output'])
...
def main():
    parser = argparse.ArgumentParser(
        description="Simple Multi-Agent ViVQA-X Evaluation")
    parser.add_argument("--config", default="../configs/vivqa_x_config.yaml",
                        help="Path to configuration file")
    parser.add_argument("--mode", type=str, default="multi-agent",
                        choices=["multi-agent", "vlm_only"],
                        help="Evaluation mode: 'multi-agent' for full pipeline, 'vlm_only' for baseline VLM performance.")
    ...
    output_dir = setup_output_directory(args, args_cmd.mode)
    print(f"Running in '{args_cmd.mode}' mode.")
    ...
    with requests.Session() as session:
        ...
        print(f"\nStarting '{args_cmd.mode}' evaluation...")
        print("="*60)
        progress_bar = tqdm(
            dataloader, desc=f"{args_cmd.mode.replace('_', ' ').title()} Processing")

        for i, data_item in enumerate(progress_bar):
            if args_cmd.mode == 'vlm_only':
                result = vlm_only_process_item(
                    data_item, args, session, verbose=args_cmd.verbose)
            elif args_cmd.mode == 'multi-agent':
                result = multi_agent_process_item(
                    data_item, args, session, verbose=args_cmd.verbose)
            results.append(result)

            if (i + 1) % 5 == 0:
                correct_so_far = sum(
                    1 for r in results if r.get('answer_correct'))
                accuracy_so_far = correct_so_far / len(results)
                tqdm.write(
                    f"Progress: {i+1}/{dataset_size}, Current Accuracy: {accuracy_so_far:.3f}")

    # Save results
    results_file = os.path.join(
        output_dir, f"vivqa_x_{args_cmd.mode}_results.json")
    ...
    try:
        ...
        evaluation_results = evaluator.evaluate_results_from_data(
            results, output_dir)
