...
class ViVQAXDataset(Dataset):
    ...
    def __init__(self, args, transform=None):
        ...
        if self.args['datasets'].get('use_num_test_data', False):
            ...
            print(f"Limited ViVQA-X dataset to {num_samples} samples for testing")
            
        print(f"Loaded {len(self.data)} samples from ViVQA-X {self.dataset_split} split")
    ...
    def __getitem__(self, idx):
        ...
        if self.args['inference']['verbose']:
            curr_data = f'image_path: {image_path} question: {question} answer: {answer} explanation: {explanation}'
            print(f'{Colors.HEADER}{curr_data}{Colors.ENDC}')
...
def get_dataloader(args):
    ...
    if args['datasets']['dataset'] == 'vivqa-x':
        dataset = ViVQAXDataset(args)
